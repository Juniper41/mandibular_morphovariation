...
#make into dataframe
pman_df <- data.frame(data.super$Csize)

#Group By Stratums
stratum <- c(rep("02", times=15), rep("07", times = 15), rep("11", times =15))
...
pman_df$stratum <- stratum

###pman_vector_df$stratum <- stratum


my_title <- expression(paste(italic("P. maniculatus")))

ggplot(pman_df, aes(group=stratum, x = stratum, y=data.super$Csize, fill=stratum)) + 
  geom_boxplot()+
  ggtitle(my_title)+
  xlab("Stratum")+
...
write.csv(pman_df,file = "pmanCS.csv")
